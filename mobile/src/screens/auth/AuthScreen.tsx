...
const AuthScreen: React.FC<AuthScreenProps> = ({ navigation, route }) => {
  ...
  const handleAuthResult = async (result: NativeAuthResult, method: 'apple' | 'google' | 'email') => {
    ...
    let linkResult = { success: false, linkedAccountId: null }; // Initialize linkResult
    ...
    if (paymentEmail && paymentEmail !== authEmail) {
      ...
      linkResult = await nativeAuthService.linkAccounts(result.userId);
  ...
  const routeAfterAuth = async (user: any, hasLinkedPayment: boolean) => {
    ...
    const currentWizardStep = journeyStore.projectWizard?.currentWizardStep;
    const wizardInProgress = currentWizardStep && currentWizardStep !== 'completed';
    ...
    // Check if user was in middle of wizard journey before auth
    if (wizardInProgress) {
      console.log('🔄 Resuming wizard journey from:', currentWizardStep);
      console.log('📊 Journey progress:', journeyProgress);
      
      // Check if we were specifically waiting for auth before AI processing
      const wasWaitingForAuth = journeyProgress.completedSteps.includes('elementSelection') && 
                               !journeyProgress.completedSteps.includes('auth');
      
      if (wasWaitingForAuth) {
        console.log('✅ Auth completed, proceeding to AI processing');
        NavigationHelpers.navigateToScreen('aiProcessing');
      } else {
        console.log('🔄 Resuming wizard at current step');
        // Resume at the current wizard step
        NavigationHelpers.navigateToScreen('aiProcessing');
      }
      return;
    }
