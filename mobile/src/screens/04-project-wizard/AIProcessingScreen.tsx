...
export const AIProcessingScreen: React.FC = () => {
  ...
  const roomAnimationProgress = new Animated.Value(0);
  ...
  // Animate room layout and progress bar as progress updates
  useEffect(() => {
    ...
    Animated.timing(roomAnimationProgress, {
      toValue: overallProgress / 100,
      duration: 1000,
      useNativeDriver: false,
    }).start();
  ...
  const checkAllRequiredDataPresent = useCallback((): boolean => {
    ...
    console.log('🎨 Style validation debug:');
    console.log('  - journeyStore.onboarding.selectedStyles:', journeyStore.onboarding?.selectedStyles);
    console.log('  - wizard.styleId:', wizard.styleId);
    console.log('  - wizard.styleName:', wizard.styleName);
    
    // Check both possible locations for style data
    const hasOnboardingStyle = journeyStore.onboarding?.selectedStyles && journeyStore.onboarding.selectedStyles.length > 0;
    const hasWizardStyle = wizard.styleId && wizard.styleName;
    
    if (!hasOnboardingStyle && !hasWizardStyle) {
      console.log('❌ Missing style selection for AI processing');
      return false;
    }
    
    console.log('✅ Style selection validated successfully:', hasOnboardingStyle ? journeyStore.onboarding.selectedStyles : [wizard.styleId]);
  ...
  const renderProgressCard = () => (
    <View style={styles.progressCard}>
      ...
      {/* Room Layout Animation */}
      <Animated.View style={[styles.roomLayoutContainer, {
        opacity: roomAnimationProgress.interpolate({
          inputRange: [0, 0.2, 1],
          outputRange: [0.3, 0.7, 1]
        })
      }]}>
        <Text style={styles.animationTitle}>AI Design Process</Text>
        <View style={styles.roomLayout}>
          {/* Room outline */}
          <View style={styles.roomOutline}>
            {/* Furniture pieces that animate in based on progress */}
            <Animated.View style={[styles.furnitureItem, styles.sofa, {
              opacity: roomAnimationProgress.interpolate({
              ...
              transform: [{
                scale: roomAnimationProgress.interpolate({
                  inputRange: [0, 0.3, 0.5],
            ...
            <Animated.View style={[styles.furnitureItem, styles.table, {
              opacity: roomAnimationProgress.interpolate({
            ...
            <Animated.View style={[styles.furnitureItem, styles.chair, {
              opacity: roomAnimationProgress.interpolate({
                inputRange: [0.4, 0.7, 1],
                outputRange: [0.3, 0.7, 1],
                extrapolate: 'clamp'
              }),
              transform: [{
            ...
            <Animated.View style={[styles.furnitureItem, styles.lamp, {
              opacity: roomAnimationProgress.interpolate({
            ...
            <Animated.View style={[styles.furnitureItem, styles.plant, {
              opacity: roomAnimationProgress.interpolate({
