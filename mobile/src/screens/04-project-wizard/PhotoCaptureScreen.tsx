...
const PhotoCaptureScreen: React.FC<PhotoCaptureScreenProps> = ({ navigation, route }) => {
  ...
  useEffect(() => {
    journeyStore.setCurrentStep(1, 'photocapture');
  }, []);
  ...
  const handleContinue = () => {
    if (capturedImage) {
      // After photo capture, go to space definition screen (step 3 of 10)
      console.log('📸 Photo captured, proceeding to function selection');
      
      if (navigation?.navigate) {
        navigation.navigate('spaceDefinition', {
          projectName,
          roomType,
          selectedStyle,
          budgetRange,
